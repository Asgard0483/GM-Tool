...
function toMarkdown(payload: ReturnType<typeof buildExportPayload>): string {
  ...
  if (data.characters) {
    ...
    data.characters.forEach(c => {
      lines.push(`### ${c.name}`, `**Typ:** ${c.typ} | **Status:** ${c.status}`, '');
      if (c.kurzbeschreibung) lines.push(c.kurzbeschreibung, '');
      if (c.persoenlichkeit) lines.push(`**Persönlichkeit:** ${c.persoenlichkeit}`, '');
      if (c.ziele) lines.push(`**Ziele:** ${c.ziele}`, '');
      if (c.hintergrund) lines.push(`**Hintergrund:** ${c.hintergrund}`, '');
      lines.push('---', '');
    });
  }
  ...
  if (data.maps) {
    ...
    data.maps.forEach(m => {
      ...
      m.pins.forEach(p => lines.push(`- ${p.label} (x: ${Math.round(p.x)}%, y: ${Math.round(p.y)}%)`));
      lines.push('', '---', '');
    });
  }
  return lines.join('\n');
...
function toCSV(payload: ReturnType<typeof buildExportPayload>): string {
  ...
  if (payload.data.characters) {
    ...
    payload.data.characters.forEach(c =>
      sections.push(`"${c.name}","${c.typ}","${c.status}","${c.volk_spezies}","${c.beruf_klasse}","${c.herkunft}"`)
    );
    sections.push('');
  }
