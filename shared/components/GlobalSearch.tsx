...
import { Search, Users, Globe, Swords, GitBranch, X, ArrowRight } from 'lucide-react';
...
export default function GlobalSearch() {
  ...
  const search = useCallback((q: string): SearchResult[] => {
    ...
    cChars.forEach(c => {
      if (c.name.toLowerCase().includes(ql) || c.kurzbeschreibung.toLowerCase().includes(ql) || c.tags.some(t => t.includes(ql))) {
        results.push({ id: c.id, entityType: 'character', title: c.name, summary: c.kurzbeschreibung, tags: c.tags, route: getRoute('character', c.id) });
      }
    });
