  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.stats}>
        ...
        {TYPES.map(v => {
          const count = campaignCharacters.filter(c => c.type === v).length;
      ...
      <div className={styles.content}>
        {filtered.length === 0 ? (
          <EmptyState
            icon={<Users size={48} opacity={0.2} />}
      ...
      <tbody>
        {characters.map(c => (
          <tr key={c.id} onClick={() => onSelect(c.id)} className={styles.tableRow}>
            ...
            <td><Badge variant={characterTypeBadge(c.type)}>{t(`types.char.${c.type}`)}</Badge></td>
            <td className={styles.muted}>{c.role || '—'}</td>
            <td className={styles.muted}>{c.species || '—'}</td>
        ...
        <div key={c.id} className={styles.card} onClick={() => onSelect(c.id)}>
          ...
          <div className={styles.cardBody}>
            ...
            <div className={styles.cardMeta}>
              <Badge variant={characterTypeBadge(c.type)} size="sm">{t(`types.char.${c.type}`)}</Badge>
              {c.species && <span className={styles.muted}>{c.species}</span>}
            </div>
            {c.short_description && (
              <p className={styles.cardDesc}>{c.short_description.slice(0, 100)}{c.short_description.length > 100 ? '…' : ''}</p>
            )}
            <div className={styles.tagList}>
